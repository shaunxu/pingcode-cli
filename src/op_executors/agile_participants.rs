...
impl OpExecutor for AgileParticipantsListOpExecutor {
    fn on_execute<'a>(&self, matches: &'a ArgMatches, _context: &OpContext) -> Result<OpRequest<'a>, AnyError> {
        Ok(OpRequest {
            ...
            query: None,
            body: None,
            parents: Some(ArgParser::parse_parents(matches, vec![("work_items", "work-item-id")])),
        })
    }
}
...
impl OpExecutor for AgileParticipantsAddOpExecutor {
    fn on_execute<'a>(&self, matches: &'a ArgMatches, _context: &OpContext) -> Result<OpRequest<'a>, AnyError> {
        ...
        json.insert(String::from("user_id"), serde_json::Value::String(String::from(matches.value_of("uid").unwrap())));
        Ok(OpRequest {
